...
class Adaptive_MinMax( layers.Layer ):
    def __init__(self , dim , features , **kwargs):
        super( Adaptive_MinMax , self ).__init__( **kwargs )
        self.dim = dim
    ...
    def call(self , inputs):
        ...
        minimum = backend.min( inputs , axis=2 )
        adaptive_min = self.shifting_layer( minimum )
        adaptive_min = backend.reshape( adaptive_min , (-1 , self.features , 1) )
        inputs -= adaptive_min

        maximum = backend.max( inputs , axis=2 )
        scale_operator = self.scaling_layer( maximum - minimum )
...
model = Sequential()
model.add(Adaptive_MinMax(12,6))
